...
- **`chipRound(team)`** — display-only wrapper around `displayRound`. Upgrades round 0 → 1 (R32) for teams that appear in any KO match in `allNormalized` but haven't won one yet (i.e. qualified but not yet played). Used for all chip rendering (draft order team chips, alive grid, owner card modal) so those teams show "R32" instead of "Round". `calcScore` still calls `displayRound` directly so scoring is unaffected.
